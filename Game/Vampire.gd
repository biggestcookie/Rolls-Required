	...
	if parried:
		...
		if cursed:
			...
			if health > max_health:
				health = max_health
			get_node("Node2D").on_health_update(health, max_health)
			Events.emit_signal("text_log_push", "[b]{name}[/b]'s damage [shake rate=30 level=15]doubles[/shake] because of your [color=blue]curse[/color] and [shake rate=30 level=15][color=red]steal your health[/color][/shake]!".format({"name":name}))	
